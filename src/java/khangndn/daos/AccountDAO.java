...
//    }
    public boolean insert(AccountDTO dto) throws Exception
    {
        ...
        try
        {
            String sql="Insert into Account(Username, Password, Fullname, Role, School, Class, Email, Phone, Status) values(?,?,?,?,?,?,?,?,?)";
            conn= MyConnection.getMyConnection();
            preStm=conn.prepareStatement(sql);
            ...
            preStm.setString(5, dto.getSchool());
            preStm.setString(6, dto.getClassName());
